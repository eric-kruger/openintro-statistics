...
Researchers often include these reference tables, called a \term{codebook}, because they orient the researcher to the variables in the dataset. Table~\ref{email50Variables} is an example of a codebook for the email50 dataset. In addition to a brief description, a codebook also can contain the variable type (discussed in the next section) and other important information related to the variable (meanings of levels of specific variables, e.g., OR = Oregon).
...
Data matrices are called different things in different contexts, sometimes they are called: a \term{spreadsheet}, \term{database}, \term{dataset} or just data! The name is not necessarily important as long as you understand what the underlying variables mean, which comes from the \term{codebook}.
...
\noindent\Large{\textbf{\GSvideohref{youtube-UW96C1H7n9o&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm&index=1} {\color{ForestGreen} Google Sheets: Database, Data matrix and Variables}}}
\normalsize
...
\term{Data visualizations} are graphical summaries of data and we'll be exploring several different types of visualizations in this chapter (scatter plot, box plot, histogram, etc.). Data visualizations are also called graphs or charts in other contexts. The importance of visualizations cannot be understated, they allow researchers to summarize complex multidimensional data in ways that would be difficult via text number summaries. It is important to remember that data visualizations are not a replacement for the analyses that you will run but are a powerful adjunct to them. They communicate your results in ways that would be difficult to do so otherwise.
...
\noindent\Large{\textbf{\GSvideohref{youtube-cP5I6jNv3Yk&index=2&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm} {\color{ForestGreen} Google Sheets: scatter plots, associations between variables}}}
\normalsize
...
Related to the concepts of \term{population} and \term{sample} are the concepts of \term{parameter} and \term{statistic}. To illustrate the distinction between the two let's use an example: a researcher wants to understand some personality characteristics of the undergraduate students. They know that across all undergraduate students in the US that 51.4\% of them are female. However, because the researcher samples on a subset of the \term{population} the \term{sample} that he obtains only contains 48\% females. In this case, the \term{parameter} is 51.4\%, the proportion of females enrolled in undergraduate education in the US, and 48\% is the \term{statistic}. In other words, the \term{parameter} is derived from the population and the sample is derived from the \term{sample}.
...
Often we choose an \term{explanatory} or \term{response} variables based on the underlying theory (research question) that we are interested in answering. Statistical tests are set up for interpretation based on the relationship between two variables. The \term{explanatory} variable is the variable we aim to explain  the \term{response} variable. Therefore, even if we are not making specific causal inferences it can be still be important to think of which variable are you using to explain another variable. Sometimes the \term{explanatory} variable is also described as a \term{predictor} variable.
...
\noindent\Large{\textbf{\GSvideohref{youtube-nNyo-z8Xt64&index=3&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm} {\color{ForestGreen} Google Sheets: mean and using functions}}}
\normalsize
...
\begin{figure}[h]
   \centering
   \includegraphics[width=0.9\textwidth]{ch_intro_to_data/figures/singleBiMultiModalPlots/singleBiMultiModalPlots}
...
\noindent\Large{\textbf{\GSvideohref{youtube-dO3eIZNmyUw&index=4&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm} {\color{ForestGreen} Google sheets: Histograms and shapes of the distribution}}}
\normalsize
...
\noindent\Large{\textbf{\GSvideohref{youtube-IkSwpaC5fl0&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm&index=5} {\color{ForestGreen} Google sheets: variance and standard deviation}}}
\normalsize
...
\noindent\Large{\textbf{\GSvideohref{youtube-1XfDcaI7Psc&index=6&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm} {\color{ForestGreen} Google sheets: box plot and 5 number summary}}}
\normalsize
...
\noindent\Large{\textbf{\GSvideohref{youtube-QhAI-4VIAzA&index=7&list=PLRevl1PsisXQ3YhbVSNoOwE9007uYn2Sm} {\color{ForestGreen} Google sheets: mosaic plot and contingency tables}}}
\normalsize
